  ...
  <v-card title="班组人员管理">
    <v-card-text>
      ...
      <v-table density="compact">
        <thead><tr><th>用户名</th><th>姓名</th><th>班组</th><th>角色</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="r in list" :key="r.username">
            <td>{{ r.username }}</td><td>{{ r.real_name }}</td><td>{{ r.shift }}</td><td>{{ ROLE_REVERSE[r.role] || r.role }}</td>
            <td>
    ...
    <v-dialog v-model="dialog" max-width="450">
      <v-card :title="editing ? '编辑人员' : '新增人员'">
        <v-card-text>
          <v-text-field v-model="form.username" label="用户名" :disabled="editing" />
          <v-text-field v-model="form.real_name" label="姓名" @update:model-value="v => { if (!editing) form.username = v }" />
          <v-select v-model="form.shift" :items="['A班', 'B班']" label="所属班组" />
    ...
    <v-dialog v-model="confirmDialog" max-width="400">
      <v-card title="确认删除">
        <v-card-text>确定删除该人员吗？此操作不可撤销。</v-card-text>
        <v-card-actions>
          <v-spacer />
...
const ROLE_MAP = { '普通操作工': 'operator', '班长': 'workshop', '小师傅': 'admin' }
const ROLE_REVERSE = Object.fromEntries(Object.entries(ROLE_MAP).map(([k, v]) => [v, k]))
const roleOptions = Object.keys(ROLE_MAP)

const list = ref([])
const form = ref({ username: '', real_name: '', shift: 'A班', role: '普通操作工', password: '123456' })
...
function openAdd() {
  form.value = { username: '', real_name: '', shift: 'A班', role: '普通操作工', password: '123456' }
  editing.value = false
  dialog.value = true
}
async function submit() {
  if (!form.value.username?.trim()) { console.error('用户名不能为空'); return }
  loading.value = true
  try {
